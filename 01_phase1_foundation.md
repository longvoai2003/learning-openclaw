...
#### What You'll Build
A customized OpenClaw configuration with adjusted session settings and personalized system prompt instructions using workspace markdown files.

#### What You'll Learn
- Editing `openclaw.json` safely for system settings
- Understanding the OpenClaw workspace architecture
- Adding custom instructions to your assistant's personality via `IDENTITY.md` and `SOUL.md`
...
3. **Understand the Workspace and Set Your Persona:**
   OpenClaw reads agent personalities from Markdown files inside your `workspace/` folder, not from the JSON config. This is a multi-file architecture:
   - `AGENTS.md` is the system ruleset (it tells the AI to read your personality).
   - `IDENTITY.md` handles the **structural metadata** (Name, Emoji, Avatar).
   - `SOUL.md` handles the **behavioral core** (How it acts and responds).

   **First, set your structural identity:**
   ```bash
   ...
   - **Vibe:** Friendly, concise, and honest
   - **Emoji:** 🐾
   ```

   **Next, define the behavioral soul:**
   ```bash
...
4. **Save both files and restart the Gateway:**
   ```bash
...
#### How to Test It
- ✅ Your assistant responds according to your custom `SOUL.md` instructions
